...
export default function AboutPage() {
    return (
        <main className="relative min-h-screen py-24 bg-black overflow-hidden px-4">

            <Container className="relative z-10">
                {/* Header Section */}
                <motion.div
                    className="max-w-5xl mx-auto mb-16 md:mb-32 text-center"
                    initial={{ opacity: 0, y: 20 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true }}
                    transition={{ duration: 0.8 }}
                >
                    <span className="inline-block px-4 py-1.5 rounded-full border border-white/10 bg-white/[0.05] text-[10px] md:text-xs font-bold tracking-[0.2em] uppercase text-neutral-400 mb-8">
                        About The Percentage Company
                    </span>
                    <h1 className="text-4xl sm:text-7xl lg:text-8xl font-bold mb-8 tracking-tight leading-[1.1] md:leading-[0.95]">
                ...
                >
                    <CardSpotlight
                        className="p-8 md:p-16 border-white/5 bg-white/[0.01]"
                    >
                        <div className="relative z-20 space-y-8 md:space-y-10">
                            <p className="text-xl md:text-3xl text-white leading-relaxed font-light">
                                We manage e-commerce operations for <span className="font-semibold text-neutral-200">jewellery and retail brands</span> that demand excellence.
                            </p>
                            ...
                            <div className="pt-6 border-t border-white/5">
                                <p className="text-sm md:text-base text-neutral-500 font-medium uppercase tracking-wider">
                    ...
                    >
                        {values.map((value, index) => (
                            <motion.div key={index} variants={fadeInUp}>
                                <CardSpotlight
                                    className="h-full border-white/5 p-6 md:p-8 bg-white/[0.01] active:scale-[0.98] transition-transform"
                                >
                                    <div className="relative z-20">
                                        <div className="flex items-start justify-between mb-6">
                                            <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-white/[0.05] border border-white/10 flex items-center justify-center">
                                                <span className="text-xs md:text-sm font-mono text-neutral-400">{value.icon}</span>
                                            </div>
                                        </div>
